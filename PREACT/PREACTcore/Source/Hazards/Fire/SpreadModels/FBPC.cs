...
namespace PREACT.Fire
{
    ...
    public class FBPC
    {
        ...
        public class FuelCoefficient
        {
            ...
            public double bui0 { get; set; }
            public double cbh { get; set; }
            public double cfl { get; set; }
        }

        public class InputSet
        {
            public string fueltype { get; set; } = "C1";
            public double ffmc { get; set; } = 85;
            public double ws { get; set; } = 0;
            public double bui { get; set; } = 12;
            public double lat { get; set; } = 50;
            public double lon { get; set; } = 110;
            public double wdir { get; set; } = 0;
            public double waz { get; set; } = 0;
            public double ps { get; set; } = 0;
            public double saz { get; set; } = 0;
            public double pc { get; set; } = 0;
            public double pdf { get; set; } = 0;
            public double cur { get; set; } = 0;
            public double gfl { get; set; } = 0;
            public double elev { get; set; } = 0;
            public double time { get; set; } = 0;
            public int jd { get; set; } = 0;

            private InputSet(InputSet seed)
            {
        ...
        private List<FuelCoefficient> setup_const()
        {
            List<FuelCoefficient> fuel_coefficients = new List<FuelCoefficient>(NumFuels);
            /*   fuel type 0 */
            fuel_coefficients.Add(new FuelCoefficient());
        ...
        private double ffmc_effect(double ffmc)
        {
            double mc, ff;
        ...
        private double rate_of_spread(x1, FuelCoefficient fuels, InputSet mains)
        {
            ...
            if (_mains.wsv < 40.0)
            {
                fw = Mathd.Exp(0.05039 * _mains.wsv);
            }
            else fw = 12.0 * (1.0 - Mathd.Exp(-0.0818 * (_mains.wsv - 28)));
        ...
        private void ros_calc(x1, FuelCoefficient fuels, isi, mult)
        {
            var ros;
        ...
        private void grass(FuelCoefficient fuels, cur, isi, mult)
        {
            var mu, ros;
            if (cur * 1.0 >= 58.8) mu = 0.176 + 0.02 * (cur * 1.0 - 58.8);
            else mu = 0.005 * (Mathd.Exp(0.061 * cur) - 1.0);
            ros = mu * (fuels.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels.b * isi)), fuels.c));
        ...
        private void mixed_wood(FuelCoefficient fuels, isi, mu, pc)
        {
            ...
            var i;
            mu = pc / 100.0;
            ros_c2 = fuels.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels.b * isi)), fuels.c);
            ...
            ros_d1 = fuels_d.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels_d.b * isi)), fuels_d.c);
        ...
        private void dead_fir(FuelCoefficient fuels, pdf, isi, mu)
        {
            var a, b, c;
            var i;
            ...
            rosm3or4_max = fuels.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels.b * isi)), fuels.c);
        ...
        private double D2_ROS(FuelCoefficient fuels, double isi, double bui, double mu)
        {
            mu = 1.0;
            _fbpc_mult = mu; // for inheritance checking!
            if (bui >= 80)
            {
                return fuels.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels.b * isi)), fuels.c);
            }
        ...
        private double conifer(FuelCoefficient fuel, double isi, double mu)
        {
            mu = 1.0;
            _fbpc_mult = mu;
            return fuel.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuel.b * isi)), fuel.c);
        }

        // ROS MODIFIERS

        private void bui_effect(FuelCoefficient fuels, at, double bui)
        {
            // modify _mains (at) by the bui effect
            var bui_avg = 50.0;

            if (bui == 0) bui = 1.0;
            _mains.be = Mathd.Exp(bui_avg * Mathd.Log(fuels.q) * ((1.0 / bui) - (1.0 / fuels.bui0)));
            return (_mains.be);
        }

        private void slope_effect(x1, fuels, at, isi)
        {
            ...
            _mains.sf = Mathd.Exp(3.533 * Mathd.Pow(ps / 100.0, 1.2));
        ...
        private void ISF_mixedwood(fuels, isz, pc, sf)
        {
            var check, mult, rsf_d1, rsf_c2, isf_d1, isf_c2;
            var i;

            rsf_c2 = sf * fuels.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels.b * isz)), fuels.c);
            if (rsf_c2 > 0.0) check = 1.0 - Mathd.Pow((rsf_c2 / (fuels.a)), (1.0 / fuels.c));
            else check = 1.0;
            if (check < FBPC_CONST.slopelimit_isi) check = FBPC_CONST.slopelimit_isi;
            isf_c2 = (1.0 / (-1.0 * fuels.b)) * Mathd.Log(check);

            if (fuels.fueltype.substr(0, 2) == "M2") mult = 0.2;
            else mult = 1.0;
            //for(i=0;strncmp(fuels.fueltype,"D1",2)!=0 && i<numfuels;fuels++,i++);
            var fuels_d1 = _fuels[get_fueltype_index("D1")];
            rsf_d1 = sf * (mult * fuels_d1.a) * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels_d1.b * isz)), fuels_d1.c);

            if (rsf_d1 > 0.0) check = 1.0 - Mathd.Pow((rsf_d1 / (mult * fuels_d1.a)), (1.0 / fuels_d1.c));
            else check = 1.0;
            if (check < FBPC_CONST.slopelimit_isi) check = FBPC_CONST.slopelimit_isi;
            isf_d1 = (1.0 / (-1.0 * fuels_d1.b)) * Mathd.Log(check);

            return (((1.0 * pc) / 100.0) * isf_c2 + (100 - (1.0 * pc)) / 100.0 * isf_d1);
        }

        private void ISF_deadfir(fuels, isz, pdf, sf)
        {
            var check, mult, rsf_d1, rsf_max, isf_d1, isf_max;
            var i;

            rsf_max = sf * fuels.a * Mathd.Pow((1.0 - Mathd.Exp(-1.0 * fuels.b * isz)), fuels.c);
            if (rsf_max > 0.0) check = 1.0 - Mathd.Pow((rsf_max / (fuels.a)), (1.0 / fuels.c));
            else check = 1.0;
            if (check < FBPC_CONST.slopelimit_isi) check = FBPC_CONST.slopelimit_isi;
            isf_max = (1.0 / (-1.0 * fuels.b)) * Mathd.Log(check);
        ...
        private void fire_intensity(fc, ros)
        {
        ...
        private void foliar_moisture(x1, at)
        {
            var latn;
            var nd;
            ...
            if (!_inputs.jd_min || _inputs.jd_min <= 0)
            {
                if (elev < 0 || !elev)
                {
                    latn = 23.4 * Mathd.Exp(-0.0360 * (150 - lon)) + 46.0;
                    _mains.jd_min = Math.floor(0.5 + 151.0 * lat / latn);
                }
                else
                {
                    latn = 33.7 * Mathd.Exp(-0.0351 * (150 - lon)) + 43.0;
        ...
        private void surf_fuel_consump(x1)
        {
            ...
            if (ft == "C1")
            {
                ...
                if (ffmc > 84) sfc = 0.75 + 0.75 * Math.sqrt(1 - Mathd.Exp(-0.23 * (ffmc - 84)));
                else sfc = 0.75 - 0.75 * Math.sqrt(1 - Mathd.Exp(0.23 * (ffmc - 84)));
            ...
            if (ft == "D1") return (1.5 * (1.0 - Mathd.Exp(-0.0183 * bui)));
            if (ft == "D2") return (bui >= 80 ? 1.5 * (1.0 - Mathd.Exp(-0.0183 * bui)) : 0.0);

            console.warn("prob in sfc func \n");
            return (-99);
        }

        private void crit_surf_intensity(fuels, fmc)
        {

            return (0.001 * Mathd.Pow(fuels.cbh * (460.0 + 25.9 * fmc), 1.5));
        }

        private void critical_ros(ft, sfc, csi)
        {
        ...
        private void crown_frac_burn(rss, rso)
        {
            var cfb;
        ...
        private void fire_type(csi, sfi)
        {

            return (sfi > csi ? 'c' : 's');
        }

        private void fire_description(cfb)
        {
            if (cfb < 0.1) return ('S');
        ...
        private void final_ros(x1, fmc, isi, cfb, rss)
        {
            var rsc, ros;
        ...
        private void foliar_mois_effect(isi, fmc)
        {
            var fme, rsc;
            var fme_avg = 0.778;
        ...
        private void crown_consump(x1, fuels, cfb)
        {
            var cfc;
            cfc = fuels.cfl * cfb;
        ...
        private void l_to_b(ft, ws)
        {
            var ws = ws * 1;
            if (ft.toUpperCase().substr(0, 2) == "O1") return (ws < 1.0 ? 1.0 : (1.1 * Mathd.Pow(ws, 0.464)));
            return (1.0 + 8.729 * Mathd.Pow(1.0 - Mathd.Exp(-0.030 * ws), 2.155));
        }

        private void set_all(firepointer, time)
        {
            firepointer.time = 0;
        ...
        private void backfire_isi(mainout)
        {
            var bfw;
            bfw = Mathd.Exp(-0.05039 * mainout.wsv);
            return (0.208 * mainout.ff * bfw);
        }

        private void backfire_ros(x1, fuels, at, bisi)
        {
            var mult = 0.0;
            var bros;
        ...
        private void perimeter(h, b, sec, lb)
        {
        ...
        private void acceleration(x1, cfb)
        {
            var i;
            var canopy = 'c';
        ...
        private void flankfire_ros(ros, bros, lb)
        {
            return ((ros + bros) / (lb * 2.0));
        }

        private void flank_spread_distance(x1, fuels, sec, hrost, brost, hd, bd, lb, a)
        {
            sec.lbt = (lb - 1.0) * (1.0 - Mathd.Exp(-a * _inputs.time)) + 1.0;
            fuels.rost = (hrost + brost) / (sec.lbt * 2.0);
            return ((hd + bd) / (2.0 * sec.lbt));
        }

        private void spread_distance(x1, fuels, a)
        {
            fuels.rost = fuels.ros * (1.0 - Mathd.Exp(-a * _inputs.time));
            return (fuels.ros * (_inputs.time + (Mathd.Exp(-a * _inputs.time) / a) - 1.0 / a));
        }

        private void time_to_crown(ros, rso, a)
        {
            var ratio;
            if (ros > 0) ratio = rso / ros;
            else ratio = 1.1;
            if (ratio > 0.9 && ratio <= 1.0) ratio = 0.9;
            if (ratio < 1.0) return Math.floor(Mathd.Log(1.0 - ratio) / -1 * a);
            else return (99);
        }

        private void fire_behaviour(x1, fuels, at, fire)
        {
        ...
        private void flank_fire_behaviour(x1, fuels, at, f)
        {
            var sfi, fi;
            var firetype;
        ...
        private void get_fueltype_index(fueltype)
        {
            var f;
            for (f in _fuels)
            {
        ...
        return -1;
		        ...
		        private void get_fueltype_number(fueltype)
        {
        ...
        private void SecondaryOutputSet()
        {
            this.lb = this.area = this.perm = this.pgr = 0.0;
        }
        private void FireOutputSet()
        {
            this.ros = 0.0; // m/min
            this.dist = 0.0; // m
        ...
        */
        ...
        private void convert_input(collection, unit)
        {
        ...
        private void convert_main(collection, unit)
        {
            /*
        ...
        private void convert_sec(collection, unit)
        {
            /*
            * Main converter does chains/hour for english/english1
            * feet/min for english2
        ...
        private void sequence_calculate()
        {
            ...
            _secs.lbt = (_secs.lb - 1.0) * (1.0 - Mathd.Exp(-accn * _inputs.time)) + 1.0;

            return true;
        }
        private void get_result_vals()
        {
            return {
                main: _mains, 
				        sec: _secs, 
        ...
        private void prepare(fueltype, ffmc, ws, bui, lat, lon, wdir, ps, saz, pc, pdf, cur, gfl, elev, time, date, jd_min, fmc)
        {
