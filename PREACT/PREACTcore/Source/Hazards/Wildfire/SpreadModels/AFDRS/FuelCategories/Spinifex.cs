...
namespace PREACT.Wildfire.AFDRS
{
    ...
    {
        private const float HEAT_CONTENT = 16700f; //'KJ/kg Malcolm Possell pers comm.
        private const float KGSQM_TO_TPH = 10f;
        private const float SECONDS_PER_HOUR = 3600f; // s
        private const float MAX_COVER = 75f; // %
...
private static float fuel_cover_spinifex(time_since_fire, subtype) As Double
...
private static float FMC_spinifex(AWAP_uf, time_since_fire, relative_humidity, air_temperature, subtype) As Double
...
private static float fuel_load_spinifex(time_since_fire, subtype) As Single
...
private static float spread_index_spinifex(wind_speed_10m, fuel_moisture, fuel_cover, wrf) As Single
...
private static float ROS_spinifex(wind_speed_10m, time_since_fire, fuel_moisture, wrf, subtype) As Double
...
private static float intensity_spinifex(rate_of_spread, time_since_fire, subtype) As Double
...
private static float flame_height_spinifex(rate_of_spread, time_since_fire, subtype) As Single
