...
namespace PREACT.Smoke
{
    public class LagrangianSmokeSim : SmokeModule
    {
        ...
        public LagrangianSmokeSim(Simulation simulation, int xDim, int yDim, int particlesPerCell) : base(simulation)
        {
            ...
            MemoryBuffer2D<Vec3, Stride2D.DenseY> canvasData = device.Allocate2DDenseY<Vec3>(new Index2D(width, height));
            MemoryBuffer1D<byte, Stride1D.Dense> d_bitmapData = device.Allocate1D<byte>(width * height * 3);
            MemoryBuffer2D<float, Stride2D.DenseX> heightmap = device.Allocate2DDenseX<float>(new Index2D(width, height));
