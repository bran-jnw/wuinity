...
namespace WUIEngine.Runtime
{
    ...
    public class RuntimeData
    {
        ...
        public bool LoadMapbox()
        {
            ...
            Mapbox.Unity.Map.MapOptions mOptions = WUInity.WUInity.MAP.Options; // new Mapbox.Unity.Map.MapOptions();
            ...
            if (!WUInity.WUInity.MAP.IsAccessTokenValid)
            {
                Engine.LOG(Engine.LogType.Error, "Mapbox token not valid.");
                return false;
            }
            Engine.LOG(Engine.LogType.Log, "Starting to load Mapbox map.");
            WUInity.WUInity.MAP.Initialize(new Mapbox.Utils.Vector2d(Engine.INPUT.Simulation.LowerLeftLatLong.x, Engine.INPUT.Simulation.LowerLeftLatLong.y), Engine.INPUT.Map.zoomLevel);
