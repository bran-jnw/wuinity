    ...
    public class CatmullRomSpline2D
    {
        ...
        public CatmullRomSpline2D(Vector3[] points, float alpha = 0.5f, float tension = 0.0f)
        {
            ...
            for (int i = 0; i < points.Length - 1; i++)
            {
                ...
                Vector2 p0_ = new Vector2(p0.Y, p0.Z);
                Vector2 p1_ = new Vector2(p1.Y, p1.Z);
                Vector2 p2_ = new Vector2(p2.Y, p2.Z);
                Vector2 p3_ = new Vector2(p3.Y, p3.Z);

                float t0 = 0.0f;
                float t1 = t0 + Mathf.Pow(Vector2.Distance(p0_, p1_), alpha);
                float t2 = t1 + Mathf.Pow(Vector2.Distance(p1_, p2_), alpha);
                float t3 = t2 + Mathf.Pow(Vector2.Distance(p2_, p3_), alpha);
