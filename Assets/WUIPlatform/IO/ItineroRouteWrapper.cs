namespace WUIPlatform
{
    ...
    public class RouteDataSave
    {
        ...
        public static EvacuationGoal GetRealEvacGoal(string name)
        {
            ...
            {
                if (WUIEngine.RUNTIME_DATA.Evacuation.EvacuationGoals[i].name == name)
                {
                    result = WUIEngine.RUNTIME_DATA.Evacuation.EvacuationGoals[i];
                }
            }

            if(result == null)
            {
                WUIEngine.LOG(WUIEngine.LogType.Warning, " While loading the route collection the evacuation goal named " + name + " did not match any of the specified evacuation goals, the route collection is therefore not valid.");
            }
