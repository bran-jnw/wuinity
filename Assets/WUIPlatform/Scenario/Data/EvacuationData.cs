...
namespace PREACT.Runtime
{
    ...
    {        
        ...
        public EvacuationData(PREACTInput input)
        {
            _input = input;
        }
        ...
        public void LoadAll(PREACTInput input, string rootFolder, out bool success)
        {
            ...
            if(input.Simulation.RunPedestrianModule)
            {
                ...
                LoadResponseCurves(rootFolder, input.Evacuation.ResponseCurveFiles, out success);
                LoadEvacuationDestinations(rootFolder, input.Evacuation.EvacuationDestinationFiles, out success);
                LoadEvacGroupFiles(rootFolder, this, input.Evacuation.EvacuationGroupFiles, out success);
                string filePath = Path.Combine(rootFolder, input.Evacuation.EvacuationGroupsMapFile);
                LoadEvacGroupIndices(filePath, out success);
                LoadBlockGoalEvents(rootFolder, input.Events.BlockGoalEventFiles, out success);
            }
