...
namespace PREACT.Farsite
{
    ...
    public class FarsiteData
    {
        ...
        private void CalcLatLong(double utmX, double utmY, string utmZone)
        {
            ...
            double e2 = Mathd.Pow((Mathd.Pow(c_sa, 2) - Mathd.Pow(c_sb, 2)), 0.5) / c_sb;
            double e2cuadrada = Mathd.Pow(e2, 2);
            double c = Mathd.Pow(c_sa, 2) / c_sb;
            double x = utmX - 500000;
            double y = isNorthHemisphere ? utmY : utmY - 10000000;

            double s = ((zone * 6.0) - 183.0);
            double lat = y / (c_sa * 0.9996);
            double v = (c / Mathd.Pow(1 + (e2cuadrada * Mathd.Pow(Mathd.Cos(lat), 2)), 0.5)) * 0.9996;
            double a = x / v;
            double a1 = Mathd.Sin(2 * lat);
            double a2 = a1 * Mathd.Pow((Mathd.Cos(lat)), 2);
            double j2 = lat + (a1 / 2.0);
            double j4 = ((3 * j2) + a2) / 4.0;
            double j6 = ((5 * j4) + Mathd.Pow(a2 * (Mathd.Cos(lat)), 2)) / 3.0;
            double alfa = (3.0 / 4.0) * e2cuadrada;
            double beta = (5.0 / 3.0) * Mathd.Pow(alfa, 2);
            double gama = (35.0 / 27.0) * Mathd.Pow(alfa, 3);
            double bm = 0.9996 * c * (lat - alfa * j2 + beta * j4 - gama * j6);
            double b = (y - bm) / v;
            double epsi = ((e2cuadrada * Mathd.Pow(a, 2)) / 2.0) * Mathd.Pow((Mathd.Cos(lat)), 2);
            double eps = a * (1 - (epsi / 3.0));
            double nab = (b * (1 - epsi)) + lat;
            double senoheps = (Mathd.Exp(eps) - Mathd.Exp(-eps)) / 2.0;
            double delt = Mathd.Atan(senoheps / (Mathd.Cos(nab)));
            double tao = Mathd.Atan(Mathd.Cos(delt) * Mathd.Tan(nab));

            llLongitude = ((delt * (180.0 / Mathd.PI)) + s) + diflon;
            llLatitude = ((lat + (1 + e2cuadrada * Mathd.Pow(Mathd.Cos(lat), 2) - (3.0 / 2.0) * e2cuadrada * Mathd.Sin(lat) * Mathd.Cos(lat) * (tao - lat)) * (tao - lat)) * (180.0 / Mathd.PI)) + diflat;
        }
